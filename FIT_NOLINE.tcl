#! /bin/env tclsh
# Fit n fake spectra with the input model for the 'source' and a power law to account for the background and outputs the result to ASCII file called 'fit_result_NOLINE_$energyline_$nspectra.dat'
# input parameters are: the number of fake spectra to be fit, the null hypothesis model and the parameters of this one
...
if {[info exists modelname]==0 ||  [info exists nspectra]==0 || [info exists params]==0} {
puts "$TAG One or more arguments are missing. Usage:xspec $argv0 -m(odel) <mod> -p(arameters) {param1 & param2 & ...} -n {number of spectra to process} -E(nergy) {line energy in keV}";exit
}
...
set fileid [open fit_result_NOLINE_$nspectra.dat w]
...
for {set i 0} {$i < $nspectra} {incr i} {
	...
	#fit background
	fitbackground $modelname $params
